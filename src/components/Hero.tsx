...
export function Hero({ initialCategories, initialSliders }: HeroProps) {
    ...
    useEffect(() => {
        setMounted(true);
        if (!initialCategories || !initialSliders) {
            fetchData();
        }
    }, [initialCategories, initialSliders]);
                    ...
                    >
                        <div className="absolute inset-0">
                            {sliders.length > 0 && sliders.map((slider, index) => (
                                <div
                                    key={slider.id}
                                    className={`absolute inset-0 p-2 transition-opacity duration-1000 ease-in-out ${index === currentSlider ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
                                >
                                    ...
                                    <div className="absolute top-1/2 left-6 md:left-14 -translate-y-1/2 z-20 text-white flex flex-col gap-1 md:gap-2 pr-4 w-full md:max-w-xl">
                                        ...
                                        <h2 className="text-[29px] sm:text-3xl md:text-5xl lg:text-6xl font-black mb-2 tracking-tight leading-tight capitalize w-[180px] h-[75px] sm:w-auto sm:h-auto overflow-hidden">
                                            {slider.title}
                                        </h2>
