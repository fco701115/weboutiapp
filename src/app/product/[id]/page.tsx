...
export default function ProductPage() {
    ...
    const fetchProduct = async (id: string) => {
        try {
            ...
            if (res.ok) {
                ...
                if (data.categoryId) {
                    fetchRelated(data.categoryId, id);
                }
            }
        } catch (error) {
    ...
    const fetchRelated = async (categoryId: string, currentId: string) => {
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                // Filter out current product and take 5
                const filtered = data
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 text-slate-400 font-bold">
                <div className="w-10 h-10 border-4 border-blue-100 border-t-blue-600 rounded-full animate-spin mr-3" />
                Cargando producto...
            </div>
        );
    }
    ...
    if (!product) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-slate-800">
                ...
                <p className="text-slate-500 font-bold uppercase tracking-widest text-[12px]">Producto no encontrado</p>
                <button onClick={() => router.push('/')} className="mt-8 bg-[#1a3da1] text-white px-8 py-3 rounded-2xl font-bold">Volver al inicio</button>
            </div>
        );
    }

    const currentImage = mainImage || 'https://images.unsplash.com/photo-1594223274512-ad4803739b7c?q=80&w=600&h=600&fit=crop';
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            >
                                ...
                                <div className="absolute bottom-6 right-6 z-40 flex flex-col gap-3">
                                    ...
                                    <div className="relative group/share">
                                        <button
                                            ...
                                            className="w-[45px] h-[45px] bg-white/90 border-2 border-white/80 backdrop-blur-md rounded-full flex items-center justify-center text-slate-400 hover:text-[#1a3da1] transition-all duration-300 shadow-lg hvr-bounce-in group-hover/share:scale-110"
                                        ...
                                        {showShareToast && (
                                            <div className="absolute right-full mr-4 top-1/2 -translate-y-1/2 bg-[#1a3da1] text-white text-[11px] font-black py-2 px-4 rounded-xl whitespace-nowrap shadow-xl animate-in fade-in slide-in-from-right-4 duration-300 flex items-center gap-2">
                                                <Check size={14} />
                                                Copiado
                                            </div>
                                        )}
                                ...
                                {/* Discount Badge */}
                                {product.salePrice && (
                                    <div className="absolute right-4 top-4 z-40 rounded-full w-[45px] h-[45px] bg-[#1a3da1] text-white flex flex-col items-center justify-center text-[11px] font-black leading-tight shadow-lg border-2 border-white animate-in zoom-in duration-500">
                                ...
                                <div className="grid grid-cols-5 gap-3">
                                    {product.images.map((img: string, i: number) => (
                                        <div
                                            ...
                                            className={`aspect-square border-2 rounded-2xl p-1 cursor-pointer transition-all ${mainImage === img ? 'border-[#1a3da1] shadow-md' : 'border-slate-100 hover:border-slate-300'}`}
                                        >
                        ...
                        <div className="flex flex-col relative py-2">
                            ...
                            <div className="flex flex-col gap-1 mb-4">
                                <span className="text-[12px] font-black text-[#1a3da1] uppercase tracking-[0.2em]">{product.category?.name || 'TECNOLOGÍA'}</span>
                            ...
                            <div className="flex items-center gap-4 mb-6">
                                <div className="flex items-baseline gap-2">
                                    <span className="text-[36px] font-black text-[#1a3da1] tracking-tighter">
                                ...
                                <span className="bg-[#1a3da1]/5 text-[#1a3da1] px-3 py-1 rounded-full text-[11px] font-black uppercase">En Stock</span>
                            </div>
                            ...
                            <div className="mt-auto space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <button
                                        ...
                                        className="h-[45px] bg-slate-900 text-white rounded-[5px] font-bold text-[14px] hover:bg-slate-800 transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-900/10 active:scale-95 px-4"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="h-[45px] bg-[#1a3da1] text-white rounded-[5px] font-bold text-[14px] hover:bg-[#153288] transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-500/10 active:scale-95 border-b-2 border-blue-900/50 px-4"
                                    >
                                ...
                                {/* Shipping Info Box */}
                                <div className="bg-blue-50/50 border border-blue-100 rounded-[20px] p-5 flex items-center gap-4 text-blue-900 shadow-sm">
                                    ...
                                    <div className="text-[12.5px] font-bold leading-tight">
                                        ...
                                        <span className="text-blue-500/60 font-medium">Recíbelo en 24-48 horas en todo el país.</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-white rounded-[5px] shadow-sm border border-slate-100 overflow-hidden mb-12">
                    <div className="flex border-b border-slate-100 bg-slate-50/50 rounded-t-[5px]">
                        <button
                            ...
                            className={`px-10 py-5 text-[13px] font-black uppercase tracking-widest transition-all ${activeTab === 'description' ? 'text-[#1a3da1] bg-white border-b-2 border-[#1a3da1]' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            Descripción
                        </button>
                         <button
